...
export default function Orders() {
  ...
  const mergeRealtimeRow = useCallback((prev: Order, row: Record<string, unknown>): Order => {
    ...
    return {
      ...
      inPreparationTimestamp: (row.in_preparation_at ?? prev.inPreparationTimestamp) as string | undefined,
      readyToShipTimestamp: (row.ready_to_ship_at ?? prev.readyToShipTimestamp) as string | undefined,
      inTransitTimestamp: (row.in_transit_at ?? prev.inTransitTimestamp) as string | undefined,
      outForDeliveryTimestamp: (row.out_for_delivery_at ?? prev.outForDeliveryTimestamp) as string | undefined,
      deliveredTimestamp: (row.delivered_at ?? prev.deliveredTimestamp) as string | undefined,
      cancelledTimestamp: (row.cancelled_at ?? prev.cancelledTimestamp) as string | undefined,
      rejectedTimestamp: (row.rejected_at ?? prev.rejectedTimestamp) as string | undefined,
      returnedTimestamp: (row.returned_at ?? prev.returnedTimestamp) as string | undefined,
      incidentTimestamp: (row.incident_reported_at ?? prev.incidentTimestamp) as string | undefined,
