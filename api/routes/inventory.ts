...
inventoryRouter.get('/movements/summary', async (req: AuthRequest, res: Response) => {
    try {
        const { date_from: dateFromParam, date_to } = req.query as { date_from?: string; date_to?: string };
        ...
        const date_from = dateFromParam || new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
