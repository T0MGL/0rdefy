-- Migration 177: 174c_unify_carrier_coverage_part3
-- Reconstructed from production DB on 2026-05-08
-- Original applied: 2026-05-08 06:02:52 UTC (version 20260508060252)
-- Part of Sprint B: carrier_zones -> carrier_coverage unification

-- 2i. process_manual_reconciliation_atomic
...
BEGIN
  ...
  IF p_total_amount_collected IS NULL THEN RAISE EXCEPTION 'total_amount_collected is required'; END IF;
  IF p_total_amount_collected < 0 THEN RAISE EXCEPTION 'total_amount_collected cannot be negative'; END IF;
  ...
  LOOP
    ...
    v_carrier_fee := COALESCE(get_carrier_fee_for_order(
      p_carrier_id,
  ...
  IF v_has_discrepancy AND array_length(v_cod_delivered_orders, 1) > 0 THEN
    ...
    LOOP
      UPDATE orders
      SET amount_collected = v_collected_amounts[i],
  ...
  INSERT INTO daily_settlements (
    ...
    net_receivable, balance_due, amount_paid,
    status, notes, created_by
  ) VALUES (
    p_store_id, p_carrier_id, v_settlement_code, p_dispatch_date,
    ...
    p_total_amount_collected, v_stats.total_cod_expected,
