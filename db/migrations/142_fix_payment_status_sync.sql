-- ================================================================
-- MIGRATION 142: Fix payment status sync for labels
-- ================================================================
-- Date: 2026-03-25
-- Purpose: Fix confirm_order_atomic RPC to allow mark_as_prepaid
--          on ANY unpaid order, not just explicit COD orders.
--
-- BUG: STEP 5 condition silently skips mark_as_prepaid for orders
--       without explicit COD indicators (payment_gateway not set,
--       cod_amount = 0). External webhook orders and manually
--       created orders fall into this gap.
--
-- CHANGE: One line in STEP 5:
--   OLD: IF p_mark_as_prepaid = TRUE AND NOT v_original_is_paid_online AND v_is_cod_order THEN
--   NEW: IF p_mark_as_prepaid = TRUE AND NOT v_original_is_paid_online THEN
--
-- Based on: exact live function (migration 104, SECURITY DEFINER, RETURNS JSON, 15 params)
...
BEGIN
  ...
  -- Verify the fix is applied: function should NOT contain 'v_is_cod_order' in the prepaid check
  SELECT EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE p.proname = 'confirm_order_atomic'
    AND n.nspname = 'public'
    AND prosrc LIKE '%Migration 142%'
  ) INTO v_has_fix;

  IF NOT v_has_fix THEN
    RAISE EXCEPTION 'FAILED: Migration 142 fix comment not found in function body';
  END IF;

  RAISE NOTICE '========================================';
  RAISE NOTICE 'Migration 142 - Fix payment status sync';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'OK: confirm_order_atomic (15 params)';
  RAISE NOTICE 'OK: v_is_cod_order removed from prepaid check';
